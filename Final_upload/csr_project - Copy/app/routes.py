...
@app.route('/send_friend_request/<ngo_id>', methods=['POST'])
def send_friend_request(ngo_id):
    if 'company_id' not in session:
        flash("You must be logged in as a company to send requests.", "error")
        return redirect(url_for('login'))
    ...
    if not company:
        flash("Company not found.", "error")
        return redirect(url_for('search_ngos'))
    ...
    if any(req['company_id'] == company_id for req in ngo.get('requests', [])):
        flash("Request already sent.", "info")
        return redirect(url_for('search_ngos'))

    # Add request to NGO collection
    request_data = {
        'company_id': company_id,
        'company_name': company['company_name']
    }
    db.ngos.update_one({'_id': ObjectId(ngo_id)}, {'$push': {'requests': request_data}})
...
def ngo_requests():
    if 'ngo_id' not in session:
        flash("You must be logged in as an NGO to view requests.", "error")
        return redirect(url_for('login'))

    ngo_id = session['ngo_id']
    ngo = db.ngos.find_one({'_id': ObjectId(ngo_id)})

    return render_template('ngo_requests.html', requests=ngo.get('requests', []))

@app.route('/accept_friend_request/<company_id>', methods=['POST'])
def accept_friend_request(company_id):
    if 'ngo_id' not in session:
        flash("You must be logged in as an NGO.", "error")
        return redirect(url_for('login'))
    ...
    # Find the request in the NGO's requests list
    request_data = next((req for req in ngo.get('requests', []) if req['company_id'] == company_id), None)
    ...
    mongo.db.ngos.update_one({'_id': ObjectId(ngo_id)}, {
        '$push': {'friends': request_data},
        '$pull': {'requests': {'company_id': company_id}}
    })

    flash("Friend request accepted!", "success")
    return redirect(url_for('ngo_requests'))

@app.route('/reject_friend_request/<company_id>', methods=['POST'])
def reject_friend_request(company_id):
    if 'ngo_id' not in session:
        flash("You must be logged in as an NGO.", "error")
        return redirect(url_for('login'))
    ...
    # Remove request from NGO's requests list
    db.ngos.update_one({'_id': ObjectId(ngo_id)}, {
        '$pull': {'requests': {'company_id': company_id}}
    })
