...
from app.ai_usp import calculate_csr_fit, get_partnership_insights
...
def match_ngo():
    ...
    if "company_email" not in session:
        print("Company email not found in session. Redirecting to login.")
    ...
    if not company:
        print(f"Company with email {session['company_email']} not found in database.")
        return "Company not found!", 404

    # Assuming you have a function to get the list of NGOs to match with the company
    print(f"Getting NGO matches for company: {company['company_name']}")
    ...
    print( matches[session["current_ngo_index"]]," SADSSSSSSSSS")
    # Get the NGO data from the session
    ngo_name = matches[session["current_ngo_index"]]  # Example of getting current NGO match name
    print(f"Selected NGO for matching: {ngo_name}")

    ngo = db.ngos.find_one({"ngo_name": ngo_name[0]})
    if not ngo:
        print(f"NGO with name {ngo_name} not found in database.")
        return "NGO not found!", 404
    
    # Calculate CSR fit score and get insights
    print(f"Calculating CSR fit score for company {company['company_name']} and NGO {ngo['ngo_name']}.")
    csr_fit_score = calculate_csr_fit(company, ngo)
    print(f"CSR Fit Score: {csr_fit_score}")

    insights = get_partnership_insights(company, ngo, csr_fit_score)
    print(f"Generated Insights: {insights}")

    # Pass the score and insights to the template
    return render_template("match_ngo.html", company=company, ngo=ngo, csr_fit_score=csr_fit_score, insights=insights)
...
def next_ngo():
    ...
    if "ngo_matches" not in session or session["current_ngo_index"] >= len(session["ngo_matches"]):
        print("No more NGO matches or ngo_matches is empty.")
        return jsonify({"status": "done"})  # No more matches

    # Fetch the company from the session
    company_email = session.get("company_email")
    if not company_email:
        print("Company is not logged in!")
        return jsonify({"status": "error", "message": "Company not logged in!"})

    # Fetch company details from the database
    company = db.companies.find_one({"email": company_email})
    if not company:
        print(f"Company with email {company_email} not found in database.")
        return jsonify({"status": "error", "message": "Company not found!"})

    # Get the current NGO match and increment the index
    ngo_name, score = session["ngo_matches"][session["current_ngo_index"]]
    session["current_ngo_index"] += 1
    print(f"Serving next NGO: {ngo_name}, CSR score: {score}")

    # Fetch NGO details from the database
    ngo_details = db.ngos.find_one({"ngo_name": ngo_name}, {"_id": 0})
    if not ngo_details:
        print(f"NGO with name {ngo_name} not found in database.")
        return jsonify({"status": "error", "message": f"NGO {ngo_name} not found!"})

    # Calculate CSR fit score and generate insights
    csr_fit_score = calculate_csr_fit(company, ngo_details)
    insights = get_partnership_insights(company, ngo_details, csr_fit_score)
    print(f"CSR Fit Score: {csr_fit_score}")
    print(f"Generated Insights: {insights}")
